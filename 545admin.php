...
if (!$pagina_login && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'atualizar_palpite') {
    ...
    if ($rodadaNome && $apostadorNome && $jogoId && $novoPalpiteTime1 !== null && $novoPalpiteTime2 !== null && is_numeric($novoPalpiteTime1) && is_numeric($novoPalpiteTime2)) {
        ...
        if ($resultadoUpdate) {
            header('Location: admin.php?ver_rodada=' . rawurlencode($rodadaNome) . '&ver_apostador=' . rawurlencode($apostadorNome) . '&update=success#palpites_apostador');
            exit;
        } else {
...
if (!$pagina_login && $_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'remover_palpite') {
    ...
    if ($rodadaNome && $apostadorNome && $jogoIdParaRemover) {
        ...
        if ($resultadoDelete) {
            header('Location: admin.php?delete_status=success_palpite' . $redirectParams);
        } else {
            header('Location: admin.php?delete_status=error_palpite' . $redirectParams);
        }
    } else {
        header('Location: admin.php?delete_status=missing_data_palpite' . ($rodadaNome ? '&ver_rodada=' . rawurlencode($rodadaNome) : '') . ($apostadorNome ? '&ver_apostador=' . rawurlencode($apostadorNome) : '') . '#palpites_apostador');
    }
    exit;
}
...
if (!$pagina_login && $_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'remover_participante') {
    ...
    if ($rodadaNome && $apostadorNomeParaRemover) {
        ...
        if ($resultadoDeleteParticipante) {
            ...
            header('Location: admin.php?' . $redirectParams . '&delete_status=success_participante');
        } else {
            error_log("ADMIN Erro ao remover participante do Firebase.");
            header('Location: admin.php?' . $redirectParams . '&delete_status=error_participante');
        }
    } else {
        error_log("ADMIN Dados insuficientes para remover participante.");
        $fallbackRedirect = 'admin.php' . ($rodadaNome ? '?ver_rodada=' . rawurlencode($rodadaNome) . '#participantes_rodada' : '');
...
<div class="container-admin"> 
    ...
    <?php else: // Se estiver logado ?>
        ...
        <div class="admin-section">
            ...
            ?>
            <form method="POST" action="admin.php" class="login-form" style="width: auto; margin: 20px 0; padding: 15px; background-color: #f0f2f5; border: 1px solid #ddd;">
        ...
        if ($rodadasData && is_array($rodadasData)) {
            if (empty($rodadasData)) {
                ...
                foreach (array_keys($rodadasData) as $nomeRodada) {
                    echo "<li><a href='admin.php?ver_rodada=" . rawurlencode($nomeRodada) . "'>" . htmlspecialchars($nomeRodada) . "</a></li>";
                }
                echo "</ul>";
                ...
                if ($rodadaSelecionada && isset($rodadasData[$rodadaSelecionada])) {
                    ...
                    if (empty($participantesDaRodada)) {
                        ...
                        foreach (array_keys($participantesDaRodada) as $nomeParticipante) {
                            echo "<li>";
                            echo "<a href='admin.php?ver_rodada=" . rawurlencode($rodadaSelecionada) . "&ver_apostador=" . rawurlencode($nomeParticipante) . "#palpites_apostador'>" . htmlspecialchars($nomeParticipante) . "</a>";
                        ...
                        if ($apostadorSelecionado && isset($participantesDaRodada[$apostadorSelecionado]['palpites'])) {
                            ...
                            if ($jogoParaEditar && isset($palpitesDoApostador[$jogoParaEditar])) {
                                ...
                                echo "<form method='POST' action='admin.php?ver_rodada=" . rawurlencode($rodadaSelecionada) . "&ver_apostador=" . rawurlencode($apostadorSelecionado) . "#palpites_apostador' class='edit-palpite-form'>";
                                ...
                                echo " <a href='admin.php?ver_rodada=" . rawurlencode($rodadaSelecionada) . "&ver_apostador=" . rawurlencode($apostadorSelecionado) . "#palpites_apostador' class='btn btn-cancel'>Cancelar</a>"; 
                                echo "</form><br>";
                            }
                            ...
                            foreach ($palpitesDoApostador as $jogoId => $palpite) {
                                ...
                                $linkRemoverPalpite = "admin.php?action=remover_palpite&rodada_nome=" . rawurlencode($rodadaSelecionada) . "&apostador_nome=" . rawurlencode($apostadorSelecionado) . "&jogo_id=" . rawurlencode($jogoId);
                                echo "<td>";
                                echo "<a href='admin.php?ver_rodada=" . rawurlencode($rodadaSelecionada) . "&ver_apostador=" . rawurlencode($apostadorSelecionado) . "&editar_palpite=" . rawurlencode($jogoId) . "#palpites_apostador' class='btn btn-warning btn-sm' style='margin-right:5px; padding: 2px 6px;'>Editar</a>";
